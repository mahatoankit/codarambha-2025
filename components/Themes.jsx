...
const Themes = () => {
  const themes = [
    {
      icon: <FaShieldAlt className="w-12 h-12 text-[#C12A37] mb-4" />,
    ...
    {
      icon: <FaRoute className="w-12 h-12 text-[#C12A37] mb-4" />,
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 max-w-4xl mx-auto">
          {themes.map((theme, index) => (
            <div
              key={index}
              className="bg-slate-900 p-8 rounded-xl border border-slate-700 transition-all duration-300 hover:shadow-2xl hover:shadow-red-900/30 hover:-translate-y-2"
            >
